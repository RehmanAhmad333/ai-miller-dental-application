...
from sample_data import dentists, services , appointments , patient_history # type: ignore
...
patient_history_df = pd.DataFrame(patient_history)

def get_patient_pattern(user_id: int):

    # User visits history
    user_visits = patient_history_df[
        patient_history_df['userId'] == user_id
    ].copy()
